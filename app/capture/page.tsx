...
type BulkAssetRating = { conditionRating: number | null; conditionJustification: string; criticalityRating: number | null };
...
type BulkSession = { id: string; startedAt: string; total: number; assetIds: string[]; fileNames: string[]; context: Pick<SurveyContext, "project" | "building" | "floor" | "zone" | "office" | "additionalLocations" | "surveyorEmail"> };
...
const copy = {
  ar: {
    ...
    choose: "اختيار من المعرض", camera: "التقاط بالكاميرا", addMore: "إضافة صور أخرى", photoTip: "مثل Google Lens: صوّر الـNameplate بوضوح، ويمكنك إضافة صور كاملة للأصل أو زوايا أخرى لتحسين النتيجة.",
  ...
  en: {
    ...
    choose: "Choose from gallery", camera: "Take a photo", addMore: "Add more photos", photoTip: "Like Google Lens: capture the nameplate clearly, then add full asset views or other angles for a better result.", ready: "Images are ready for analysis",
    ...
    contextTitle: "Asset location", contextLead: "Choose the project and asset location before adding photos. Required fields are configured by the administrator.", project: "Project / Entity", building: "Building / Site", floor: "Floor", zone: "Zone", office: "Office / Room", chooseValue: "Choose from list", yes: "Yes", no: "No", manualEntry: "Not listed — enter manually", enterManually: "Enter value manually", required: "Required", optional: "Optional", contextRequired: "Complete the required asset-location fields before adding it to the analysis queue.", noProjects: "No projects are available for your account. Ask an administrator to add a project or grant access.", adminPanel: "Administration", reports: "Reports", logout: "Sign out",
...
const sampleResult: Result = {
  assetType: "Centrifugal Pump", summary: "Horizontal centrifugal water pump with three-phase electric motor.", overallConfidence: 0.92, conditionRating: null, criticalityRating: null,
  fields: [
...
export default function Home() {
  ...
  const [manualLocationValues, setManualLocationValues] = useState<Record<string, string>>({});
  ...
  const [manualAsset, setManualAsset] = useState({ assetType: "", summary: "", manufacturer: "", model: "", serial: "" });
  ...
  const availableOffices = selectedBuilding?.offices.filter(item => (!item.floorId || !floorId || item.floorId === floorId) && (!item.zoneId || !zoneId || item.zoneId === zoneId)) || [];
  ...
  const officeValue = officeId === "__manual__" ? manualOffice.trim() : selectedOffice?.name || "";
  ...
  const locationFields = selectedProject?.customFields.filter(field => (field.assetTypes || []).length === 0 && field.id !== gpsField?.id && field.id !== barcodeField?.id) || [];
  const activeAssetType = (result?.assetType || manualAsset.assetType).trim().toLocaleLowerCase("en").replace(/\s+/g, " ");
  ...
  const bulkRatingsComplete = !bulkMode || files.every((_, index) => {
    ...
    return Boolean(rating && Number.isInteger(rating.conditionRating) && Number(rating.conditionRating) >= 1 && Number(rating.conditionRating) <= 5 && (Number(rating.conditionRating) > 2 || rating.conditionJustification.trim().length >= 3) && Number.isInteger(rating.criticalityRating) && Number(rating.criticalityRating) >= 1 && Number(rating.criticalityRating) <= 5);
  });
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const scan = async () => { if (!active) return; try { const found = await detector.detect(video); if (found[0]?.rawValue) { active = false; await handleScannedBarcode(found[0].rawValue); setScannerOpen(false); return; } } catch { /* keep scanning the next frame */ } scannerFrameRef.current = window.requestAnimationFrame(() => void scan()); };
        await scan();
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (requestedOffice && building.offices.some(item => item.id === requestedOffice)) setOfficeId(requestedOffice);
    }, 0);
    return () => window.clearTimeout(timer);
  }, [masterConfig]);
  ...
  function acceptFiles(nextFiles: File[]) {
    ...
    setFiles(current => [...current, ...nextFiles]); setPreviews(current => [...current, ...nextPreviews]);
    setBulkRatings(current => [...current, ...nextFiles.map(() => ({ conditionRating, conditionJustification, criticalityRating }))]);
    setResult(null); setDemo(false); setError(""); setEditingRecordId(null);
  }
  ...
  function removeFile(index: number) {
    ...
    setFiles(current => current.filter((_, itemIndex) => itemIndex !== index)); setPreviews(current => current.filter((_, itemIndex) => itemIndex !== index)); setBulkRatings(current => current.filter((_, itemIndex) => itemIndex !== index)); setResult(null); setError("");
  }
  function clear() {
  ...
  function resetCaptureForm() {
    clear();
    setProjectId(""); setBuildingId(""); setFloorId(""); setZoneId(""); setOfficeId("");
    setManualBuilding(""); setManualFloor(""); setManualZone(""); setManualOffice("");
    setLocationSelections({}); setManualLocationValues({}); setEntryMode("images"); setBulkMode(false); setBulkSession(null);
  }
  ...
  function downloadBulkSessionCsv() {
    ...
    const rows = bulkSession.assetIds.map((assetId, index) => {
      ...
      const locationContext = record?.surveyContext || job?.surveyContext || bulkSession.context;
      ...
      const row: Record<string, string | number> = {
        ...
        "Criticality": assetCriticalityLabel(resultSource?.criticalityRating, "en"),
        "Asset Weight": assetCriticalityWeight(resultSource?.criticalityRating) || "",
      ...
      for (const location of locationContext.additionalLocations || []) row[location.labelEn || location.labelAr || location.key] = location.value;
      return row;
    });
    const columns = Object.keys(rows[0] || { "No.": "", "Status": "", "Asset No.": "", "Source File": "", "Project": "", "Building / Site": "", "Floor": "", "Zone": "", "Office / Room": "", "Asset Type": "", "Manufacturer": "", "Model": "", "Serial Number": "", "Surveyor": "" });
  ...
  function currentSurveyContext(position: GPSPosition | null = gps, capturedOffline = false, offlineClientId = crypto.randomUUID(), ratingOverride?: BulkAssetRating): SurveyContext | null {
    const effectiveCondition = ratingOverride?.conditionRating ?? conditionRating;
    const effectiveJustification = (ratingOverride?.conditionJustification ?? conditionJustification).trim();
    const effectiveCriticality = ratingOverride?.criticalityRating ?? criticalityRating;
  ...
  async function queueAnalysis() {
    ...
    if (!Number.isInteger(conditionRating) || Number(conditionRating) < 1 || Number(conditionRating) > 5) { setError(language === "ar" ? "اختر تقييم حالة الأصل قبل إرفاق الصور للتحليل." : "Choose the asset condition before attaching images for analysis."); setCaptureStep(1); return; }
    if (Number(conditionRating) <= 2 && conditionJustification.trim().length < 3) { setError(language === "ar" ? "اكتب سبب تقييم الأصل كحالة حرجة أو ضعيفة قبل المتابعة." : "Explain why the asset is Critical or Poor before continuing."); setCaptureStep(1); return; }
    if (!Number.isInteger(criticalityRating) || Number(criticalityRating) < 1 || Number(criticalityRating) > 5) { setError(language === "ar" ? "اختر درجة أهمية الأصل قبل إرفاق الصور للتحليل." : "Choose the asset criticality before attaching images for analysis."); setCaptureStep(1); return; }
    if (shouldQueueAsBulk && !bulkRatingsComplete) { setError(language === "ar" ? "أكمل تقييم الحالة والأهمية وسبب الحالة الضعيفة لكل صورة في جلسة Bulk." : "Complete condition, criticality and any required poor-condition reason for every Bulk image."); return; }
    ...
    setUploading(true); setError(""); setCaptureStep(2);
    const saveForSync = async () => {
      ...
      if (shouldQueueAsBulk) {
        for (const [index, file] of compressedFiles.entries()) {
          const offlineContext = currentSurveyContext(capturedPosition, true, crypto.randomUUID(), bulkRatings[index]);
    ...
    try {
      ...
      if (shouldQueueAsBulk) {
        ...
        setBulkSession({ id: sessionId, startedAt, total: compressedFiles.length, assetIds: [], fileNames: compressedFiles.map(file => file.name), context: { project: surveyContext.project, building: surveyContext.building, floor: surveyContext.floor, zone: surveyContext.zone, office: surveyContext.office, additionalLocations: surveyContext.additionalLocations, surveyorEmail: surveyContext.surveyorEmail } });
        for (const [index, image] of compressedFiles.entries()) {
          const itemContext = currentSurveyContext(capturedPosition, false, crypto.randomUUID(), bulkRatings[index]);
  ...
  async function createManualAsset() {
    if (uploading) return;
    if (!manualAsset.assetType.trim()) { setError(language === "ar" ? "اكتب نوع الأصل أولاً." : "Enter the asset type first."); return; }
    const missing = assetFields.find(field => field.required && !customValues[field.key]?.trim());
    if (missing) { setError(`${language === "ar" ? missing.labelAr : missing.labelEn || missing.labelAr} ${t.required}`); return; }
    if (!manualSubmissionIdRef.current) manualSubmissionIdRef.current = crypto.randomUUID();
    const surveyContext = currentSurveyContext(gps, false, manualSubmissionIdRef.current);
    if (!surveyContext) { setError(t.contextRequired); setCaptureStep(1); return; }
    setUploading(true); setError("");
    try {
      const token = await getAccessToken(); if (!token) { window.location.replace("/login"); return; }
      const response = await fetch("/api/assets", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ action: "createManual", context: surveyContext, ...manualAsset }) });
      const payload = await response.json() as { assetId?: string; assetNo?: string; created?: boolean; duplicate?: boolean; error?: string };
      if (!response.ok || !payload.assetId) throw new Error(payload.error || "تعذر إنشاء الأصل اليدوي.");
      invalidateApiCache("/api/dashboard"); invalidateApiCache("/api/reports"); invalidateApiCache("/api/assets?view=list");
      resetCaptureForm();
      setOfflineNotice(language === "ar" ? `تم إنشاء الأصل${payload.assetNo ? ` ${payload.assetNo}` : ""} بنجاح، وتم تصفير النموذج للبدء من جديد.` : `Asset${payload.assetNo ? ` ${payload.assetNo}` : ""} was created successfully. The form is ready for a new asset.`);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "تعذر إنشاء الأصل اليدوي."); }
    finally { setUploading(false); }
  }
  ...
  async function exportExcel() {
    ...
    const locationColumns = Array.from(new Set(exportRecords.flatMap(record => (record.surveyContext?.additionalLocations || []).map(location => location.labelEn || location.labelAr || location.key))));
    const columns = ["Asset No.", "Status", "Captured At", "Surveyor", "Project", "Building / Site", "Floor", "Zone", ...locationColumns, ...customColumns, "Source File", "Asset Type", "Asset Summary", "Condition Rating", "Condition", "Condition Justification", "Criticality Rating", "Criticality", "Asset Weight", "Overall Confidence", ...fieldColumns];
    const assetRows = exportRecords.map((record, index) => {
      const row: Record<string, string | number> = {
        ...
        "Asset Summary": record.summary, "Condition Rating": record.conditionRating || "", "Condition": assetConditionLabel(record.conditionRating, "en"), "Condition Justification": record.conditionJustification || "", "Criticality Rating": record.criticalityRating || "", "Criticality": assetCriticalityLabel(record.criticalityRating, "en"), "Asset Weight": assetCriticalityWeight(record.criticalityRating) || "", "Overall Confidence": record.overallConfidence,
      };
      for (const location of record.surveyContext?.additionalLocations || []) row[location.labelEn || location.labelAr || location.key] = location.value;
    ...
    const confidenceRows = exportRecords.map((record, index) => {
      const row: Record<string, string | number> = {
        "Asset No.": record.assetNo || index + 1, "Status": "", "Captured At": "", "Surveyor": "", "Project": "", "Building / Site": "", "Floor": "", "Zone": "", "Source File": "", "Asset Type": "", "Asset Summary": "", "Condition Rating": "", "Condition": "", "Condition Justification": "", "Criticality Rating": "", "Criticality": "", "Asset Weight": "", "Overall Confidence": record.overallConfidence,
      };
  ...
  async function addToRegister(action: "save" | "approve" = "save") {
    ...
    if (!Number.isInteger(result.conditionRating) || Number(result.conditionRating) < 1 || Number(result.conditionRating) > 5) { setError(language === "ar" ? "اختر حالة الأصل من مستويات التقييم الخمسة قبل الحفظ." : "Choose one of the five asset-condition levels before saving."); return; }
    if (Number(result.conditionRating) <= 2 && (result.conditionJustification || "").trim().length < 3) { setError(language === "ar" ? "اكتب سبب تقييم الأصل كحالة حرجة أو ضعيفة قبل الحفظ." : "Explain why the asset is Critical or Poor before saving."); return; }
    if (!Number.isInteger(result.criticalityRating) || Number(result.criticalityRating) < 1 || Number(result.criticalityRating) > 5) { setError(language === "ar" ? "اختر درجة أهمية الأصل قبل الحفظ." : "Choose the asset criticality before saving."); return; }
    try {
      const token = await getAccessToken(); if (!token) { window.location.replace("/login"); return; }
      const response = await fetch("/api/assets", { method: "PATCH", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ id: editingRecordId, result, customValues, action }) });
      const payload = await response.json() as { records: StoredRecord[]; jobs: AnalysisJob[]; error?: string };
  ...
  async function copyResult() {
    if (!result) return;
    await navigator.clipboard.writeText([`Asset Type: ${result.assetType}`, `Condition: ${result.conditionRating || "—"}/5 — ${assetConditionLabel(result.conditionRating, "en") || "Not rated"}`, result.conditionJustification ? `Condition Justification: ${result.conditionJustification}` : "", `Criticality: ${assetCriticalityLabel(result.criticalityRating, "en") || "Not selected"} — Weight ${assetCriticalityWeight(result.criticalityRating) || "—"}`, ...result.fields.map(f => `${f.label}: ${f.value}`)].filter(Boolean).join("\n"));
  ...
  function renderCustomField(field: CustomSurveyField) {
    const label = language === "ar" ? field.labelAr : field.labelEn || field.labelAr;
    return <label className="custom-survey-field" key={field.id}><span>{label}{field.unit ? ` (${field.unit})` : ""}<em>{field.required ? t.required : t.optional}</em></span>
  ...
  function startEntry(mode: "images" | "manual") {
    ...
    setEntryMode(mode); setCaptureStep(2); setError(""); setOfflineNotice(""); setResult(null); setEditingRecordId(null);
  }
  return (
    <main className="capture-page al-page" dir={language === "ar" ? "rtl" : "ltr"}>
      ...
      {captureStep === 1 && <section className="survey-context" aria-labelledby="context-title">
        ...
        {selectedProject && <div className="capture-mode-actions"><button className="al-primary-button" onClick={() => startEntry("images")}>⌾ {language === "ar" ? "متابعة للتصوير أو رفع الصور" : "Continue to photo or upload"}</button><button className="al-secondary-button" onClick={() => startEntry("manual")}>＋ {language === "ar" ? "إضافة أصل يدويًا" : "Add asset manually"}</button></div>}
      </section>}
      ...
      {captureStep > 1 && <section className="workspace capture-workspace">
        ...
        {captureStep === 2 && entryMode === "images" && <article className="panel upload-panel">
          ...
          <input ref={inputRef} hidden type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={(e: ChangeEvent<HTMLInputElement>) => { acceptFiles(Array.from(e.target.files || [])); e.target.value = ""; }} />
          <input ref={cameraInputRef} hidden type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={(e: ChangeEvent<HTMLInputElement>) => { acceptFiles(Array.from(e.target.files || [])); e.target.value = ""; }} />
          {previews.length === 0 ? <div className="dropzone" onDragOver={e => e.preventDefault()} onDrop={onDrop}>
            <div className="upload-icon"><UploadIcon /></div><h3>{t.uploadHint}</h3><p>{bulkMode ? t.bulkFormats : t.formats}</p><div className="image-source-actions"><button type="button" className="camera-source" onClick={() => cameraInputRef.current?.click()}>⌾ {t.camera}</button><button type="button" className="secondary" onClick={() => inputRef.current?.click()}>▧ {t.choose}</button></div>
          </div> : <div className="image-ready">
            <div className="image-gallery">{previews.map((preview, index) => <figure key={preview}><img src={preview} alt={`Selected asset view ${index + 1}`} /><span>{index + 1}</span><button type="button" onClick={() => removeFile(index)} aria-label={t.removePhoto}>×</button></figure>)}{files.length < (bulkMode ? BULK_ZONE_LIMIT : SINGLE_ASSET_LIMIT) && <><button type="button" className="add-photo" onClick={() => cameraInputRef.current?.click()}><b>⌾</b><span>{t.camera}</span></button><button type="button" className="add-photo" onClick={() => inputRef.current?.click()}><b>＋</b><span>{t.choose}</span></button></>}</div>
            ...
            {bulkMode && <section className="bulk-rating-list"><header><strong>{language === "ar" ? "تقييم كل أصل في جلسة Bulk" : "Rate every Bulk asset"}</strong><small>{language === "ar" ? "التقييم الموجود بالأعلى هو القيمة الابتدائية، ويمكن تعديل كل صورة منفردة." : "The rating above is the initial value; each image can be changed independently."}</small></header>{files.map((file, index) => {
              const rating = bulkRatings[index] || { conditionRating: null, conditionJustification: "", criticalityRating: null };
              return <article key={`${file.name}-${file.lastModified}-${index}`}><div className="bulk-rating-file"><b>{index + 1}</b><span title={file.name}>{file.name}</span></div><label><span>{language === "ar" ? "الحالة" : "Condition"}</span><select value={rating.conditionRating || ""} onChange={event => setBulkRatings(current => current.map((item, itemIndex) => itemIndex === index ? { ...item, conditionRating: Number(event.target.value) || null, conditionJustification: Number(event.target.value) > 3 ? "" : item.conditionJustification } : item))}><option value="">—</option>{ASSET_CONDITION_LEVELS.map(level => <option value={level.rating} key={level.rating}>{level.rating} — {language === "ar" ? level.labelAr : level.labelEn}</option>)}</select></label><label><span>{language === "ar" ? "الأهمية" : "Criticality"}</span><select value={rating.criticalityRating || ""} onChange={event => setBulkRatings(current => current.map((item, itemIndex) => itemIndex === index ? { ...item, criticalityRating: Number(event.target.value) || null } : item))}><option value="">—</option>{ASSET_CRITICALITY_LEVELS.map(level => <option value={level.rating} key={level.rating}>{level.rating} — {language === "ar" ? level.labelAr : level.labelEn}</option>)}</select></label>{rating.conditionRating !== null && rating.conditionRating <= 3 && <label className="bulk-rating-reason"><span>{language === "ar" ? "سبب الحالة" : "Condition reason"}{rating.conditionRating <= 2 && <em>{t.required}</em>}</span><input maxLength={1000} value={rating.conditionJustification} onChange={event => setBulkRatings(current => current.map((item, itemIndex) => itemIndex === index ? { ...item, conditionJustification: event.target.value } : item))} placeholder={language === "ar" ? "سبب مختصر وواضح" : "Short clear reason"} /></label>}</article>;
            })}</section>}
          </div>}
          <div className="tip"><span>◎</span><p>{t.photoTip}</p></div>{error && <div className="error" role="alert">{error}</div>}
          <button className="analyze" disabled={files.length === 0 || uploading || (bulkMode && !bulkRatingsComplete)} onClick={queueAnalysis}>{uploading ? <span className="mini-spinner" /> : <ScanIcon />}{uploading ? t.analyzing : shouldQueueAsBulk ? t.bulkAnalyze : t.analyze}</button>
        </article>}
        {captureStep === 2 && entryMode === "manual" && <article className="panel manual-asset-panel">
          <div className="panel-heading"><span>02</span><div><h2>{language === "ar" ? "إضافة بيانات الأصل يدويًا" : "Add asset details manually"}</h2><p>{language === "ar" ? "أدخل نوع الأصل ثم أكمل الحقول التي حددها مدير النظام." : "Enter the asset type, then complete the administrator-defined fields."}</p></div></div>
          <div className="manual-rating-summary"><div><span>{language === "ar" ? "حالة الأصل" : "Condition"}</span><b>{conditionRating}/5 — {assetConditionLabel(conditionRating, language)}</b></div><div><span>{language === "ar" ? "أهمية الأصل" : "Criticality"}</span><b>{assetCriticalityLabel(criticalityRating, language)} — {language === "ar" ? "الوزن" : "Weight"} {assetCriticalityWeight(criticalityRating)}</b></div>{conditionJustification && <p><span>{language === "ar" ? "سبب الحالة" : "Condition reason"}</span>{conditionJustification}</p>}</div>
          <div className="manual-asset-grid">
            <label><span>{language === "ar" ? "نوع الأصل" : "Asset type"}<em>{t.required}</em></span><input value={manualAsset.assetType} onChange={event => setManualAsset(current => ({ ...current, assetType: event.target.value }))} placeholder="Air Conditioner" /></label>
            <label><span>{language === "ar" ? "الشركة المصنعة" : "Manufacturer"}<em>{t.optional}</em></span><input value={manualAsset.manufacturer} onChange={event => setManualAsset(current => ({ ...current, manufacturer: event.target.value }))} /></label>
            <label><span>{language === "ar" ? "الموديل" : "Model"}<em>{t.optional}</em></span><input value={manualAsset.model} onChange={event => setManualAsset(current => ({ ...current, model: event.target.value }))} /></label>
            <label><span>{language === "ar" ? "السيريال" : "Serial number"}<em>{t.optional}</em></span><input value={manualAsset.serial} onChange={event => setManualAsset(current => ({ ...current, serial: event.target.value }))} /></label>
            <label className="manual-summary"><span>{language === "ar" ? "وصف مختصر" : "Summary"}<em>{t.optional}</em></span><textarea value={manualAsset.summary} onChange={event => setManualAsset(current => ({ ...current, summary: event.target.value }))} /></label>
            {assetFields.map(renderCustomField)}
          </div>
          {manualAsset.assetType.trim() && assetFields.length === 0 && <p className="manual-field-note">{language === "ar" ? "لا توجد حقول إضافية مخصصة لهذا النوع. يمكنك إضافتها من إدارة النظام." : "No additional fields are configured for this asset type."}</p>}
          {error && <div className="error" role="alert">{error}</div>}
          <button className="analyze" disabled={!manualAsset.assetType.trim() || uploading} onClick={() => void createManualAsset()}>{uploading ? <span className="mini-spinner" /> : "✓"}{language === "ar" ? "إنشاء الأصل" : "Create asset"}</button>
        </article>}
        {captureStep === 3 && <article className="panel result-panel">
          ...
          <div className="results">
            <div className="result-summary"><div><small>Asset Type</small><input className="result-asset-type" value={result.assetType} onChange={event => setResult(current => current ? { ...current, assetType: event.target.value } : current)} placeholder="Asset type" /><p>{result.summary}</p></div><div className="score" style={{ "--score": `${Math.round(result.overallConfidence * 100) * 3.6}deg` } as CSSProperties}><span>{Math.round(result.overallConfidence * 100)}%</span><small>{t.confidence}</small></div></div>
            <div className="review-condition-summary"><strong>{language === "ar" ? "حالة الأصل" : "Asset condition"}</strong><span>{result.conditionRating || "—"}/5 — {assetConditionLabel(result.conditionRating, language) || (language === "ar" ? "غير محددة" : "Not selected")}</span></div>
            {result.conditionRating !== null && result.conditionRating !== undefined && result.conditionRating <= 3 && <label className="condition-justification review-justification"><span>{language === "ar" ? "سبب تقييم الحالة" : "Condition justification"}<em>{result.conditionRating <= 2 ? t.required : t.optional}</em></span><textarea maxLength={1000} value={result.conditionJustification || ""} onChange={event => setResult(current => current ? { ...current, conditionJustification: event.target.value } : current)} placeholder={language === "ar" ? "اكتب سبب الحالة والإجراء المقترح." : "Explain the condition and suggested action."} /></label>}
            <div className="review-condition-summary criticality"><strong>{language === "ar" ? "أهمية الأصل" : "Asset criticality"}</strong><span>{assetCriticalityLabel(result.criticalityRating, language) || (language === "ar" ? "غير محددة" : "Not selected")} — {language === "ar" ? "الوزن التلقائي" : "Automatic weight"} {assetCriticalityWeight(result.criticalityRating) || "—"}</span></div>
