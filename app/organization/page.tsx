...
export default function OrganizationPage() {
  ...
  return <section className="al-page organization-page">
    <header className="al-page-head"><div><span className="al-page-kicker">Organization map</span><h2>هيكل واضح لكل مشروع وموقع</h2><p>اعرض العلاقة بين المشروع، المبنى، الطابق والزون في صفحة مستقلة سهلة القراءة.</p></div><div className="al-page-actions"><Link className="al-secondary-button" href="/locations">عرض المواقع</Link>{data?.currentUser.role === "admin" && <Link className="al-primary-button" href="/admin#projects">إدارة الهيكل</Link>}</div></header>
    ...
    {!data && !error ? <div className="al-loading-grid"><div className="al-skeleton"/><div className="al-skeleton"/><div className="al-skeleton"/><div className="al-skeleton"/></div> : data && <>
      <div className="organization-summary">{[[totals.projects,"المشاريع","01"],[totals.buildings,"المباني والمواقع","02"],[totals.floors,"الطوابق","03"],[totals.zones,"الزونات","04"]].map(([value,label,index]) => <article className="al-card" key={String(label)}><span>{index}</span><div><strong>{value}</strong><small>{label}</small></div></article>)}</div>
      {data.projects.length ? <div className="organization-projects">{data.projects.map((project,index) => {
        ...
        return <article className="al-card organization-project-card" key={project.id}><header><div><span>PROJECT {String(index+1).padStart(2,"0")}</span><h3>{project.name}</h3></div><b>{project.buildings.length} مواقع</b></header><div className="organization-project-stats"><div><strong>{project.buildings.length}</strong><span>مبنى</span></div><div><strong>{floorCount}</strong><span>طابق</span></div><div><strong>{zoneCount}</strong><span>زون</span></div></div><div className="organization-rules"><span className={project.requireBuilding ? "required" : ""}>المبنى {project.requireBuilding ? "إلزامي" : "اختياري"}</span><span className={project.requireFloor ? "required" : ""}>الطابق {project.requireFloor ? "إلزامي" : "اختياري"}</span><span className={project.requireZone ? "required" : ""}>الزون {project.requireZone ? "إلزامي" : "اختياري"}</span></div><div className="organization-building-list">{project.buildings.length ? project.buildings.map(building => <div key={building.id}><span className="organization-node-icon">⌂</span><div><strong>{building.name}</strong><small>{building.floors.length} طوابق · {building.zones.length} زونات</small></div><Link href={`/locations?project=${project.id}&building=${building.id}`}>عرض</Link></div>) : <p>لم تتم إضافة مواقع لهذا المشروع بعد.</p>}</div></article>;
