...
type AssetRow = {
  ...
  overall_confidence: number; status: string; error: string | null; created_at: string;
...
type ProjectRow = { id: string; name: string; require_building: boolean; require_floor: boolean; require_zone: boolean };
type ConfigRow = { id: string; project_id: string; status: string };
type CustomFieldRow = { id: string; config_id: string; field_key: string; label_ar: string; label_en: string; required: boolean; enabled: boolean };
...
type ActorRow = { id: string; role: "admin" | "surveyor"; active: boolean };
type ImportRow = {
  assetNo?: unknown; assetType?: unknown; manufacturer?: unknown; model?: unknown; serial?: unknown; summary?: unknown;
  building?: unknown; floor?: unknown; zone?: unknown; customValues?: unknown;
};
...
async function reportAssets(token: string, projectFilter: string) {
  const baseFields = "id,asset_no,project_id,survey_config_id,created_by,project_name,building_name,floor_name,zone_name,surveyor_email,source_file_names,asset_type,summary,fields,warnings,overall_confidence,status,error,created_at";
  try {
...
async function reportData(token: string, profile: ActorRow) {
  ...
  const [assets, projects, customFields, customValues] = await Promise.all([
    reportAssets(token, projectFilter),
    supabaseRest<ProjectRow[]>(`projects?select=id,name,require_building,require_floor,require_zone&active=eq.true&${projectFilter}&order=name`, token),
    supabaseRest<CustomFieldRow[]>("custom_fields?select=id,config_id,field_key,label_ar,label_en,required,enabled&enabled=eq.true&order=sort_order&limit=10000", token),
  ...
  const records = assets.map(asset => {
    ...
    const custom = values.map(value => {
      ...
      return field ? { key: field.field_key, labelAr: field.label_ar, labelEn: field.label_en, value: text(value.value_text, 1000) } : null;
    }).filter(Boolean);
    ...
    for (const field of requiredByConfig.get(asset.survey_config_id || "") || []) if (!valueByField.get(field.id)) missingFields.push(field.label_en || field.label_ar);
    return {
      id: asset.id, assetNo: asset.asset_no, projectId: asset.project_id, project: asset.project_name,
      building: asset.building_name, floor: asset.floor_name, zone: asset.zone_name, surveyorEmail: asset.surveyor_email,
      ...
      status: asset.status, error: asset.error || "", createdAt: asset.created_at, isDuplicate: duplicateWarning(asset.warnings), missingFields,
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json(await reportData(token, profile), { headers: { "Cache-Control": "private, max-age=20, stale-while-revalidate=60" } });
...
export async function POST(request: Request) {
  try {
    ...
    const [projects, configs, allFields, existingAssets] = await Promise.all([
      supabaseRest<ProjectRow[]>(`projects?select=id,name,require_building,require_floor,require_zone&id=eq.${encodeURIComponent(projectId)}&active=eq.true&limit=1`, token),
      ...
      supabaseRest<CustomFieldRow[]>("custom_fields?select=id,config_id,field_key,label_ar,label_en,required,enabled&enabled=eq.true&limit=10000", token),
    ...
    const prepared = rows.map((row, index) => {
      ...
      return {
        ...
        building_name: text(row.building), floor_name: text(row.floor), zone_name: text(row.zone), surveyor_email: user.email,
        ...
        raw_text: "", overall_confidence: 1, status: "review", error: null,
      };
    });
    ...
    return Response.json({ imported: inserted.length, report: await reportData(token, profile) }, { status: 201 });
