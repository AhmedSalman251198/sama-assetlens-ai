...
test("planned filters are scoped to real values and only count matching records", () => {
  const plan = validateAskQueryPlan(
    { building: "b1", criticalityMin: 5, conditionMax: 2 },
    assets,
  );
  assert.deepEqual(
    applyAskQueryPlan(assets, plan).map((asset) => asset.assetNo),
    ["AC-01"],
  );
  assert.equal(
    askAssetMetrics(applyAskQueryPlan(assets, plan)).combinedRiskScore,
    25,
  );
  assert.throws(
    () => validateAskQueryPlan({ building: "another project" }, assets),
    /Unknown building/,
  );
  assert.throws(
    () => validateAskQueryPlan({ conditionMin: 5, conditionMax: 1 }, assets),
    /Contradictory/,
  );
  assert.deepEqual(
    applyAskQueryPlan(assets, { conditionMax: 2 }).map(
      (asset) => asset.assetNo,
    ),
    ["AC-01", "AC-02"],
  );
...
test("AI answer uses computed full-result counts and discards citations outside authorized evidence", async () => {
            ...
            parts: [
              {
                text: JSON.stringify(
                  calls === 1
                    ? { building: "B1", criticalityMin: 5, conditionMax: 2 }
                    : {
                        answer: "يوجد أصل واحد حرج في B1 وحالته ضعيفة.",
                        citations: [assets[0].id, assets[2].id],
                        caveat: "",
                      },
                ),
              },
  ...
  try {
    const result = await answerAssetLens(
      "كم أصلًا حرجًا حالته ضعيفة في B1؟",
      assets,
      "ar",
    );
...
test("missing or unavailable AI keeps analytical calculations separate from explicit register search", async () => {
  const previousKey = process.env.GEMINI_API_KEY;
  const originalFetch = globalThis.fetch;
  delete process.env.GEMINI_API_KEY;
  try {
    const fallback = await answerAssetLens("critical assets", assets, "en");
    assert.equal(fallback.mode, "calculation");
    assert.equal(fallback.totalMatches, 2);
    assert.match(fallback.limitation, /calculated directly from the asset register/);
    assert.equal(
      askAssetLens("find asset unrecognized-nonexistent-type", assets).totalMatches,
      0,
    );
    process.env.GEMINI_API_KEY = "test-key";
    globalThis.fetch = async () =>
      Response.json(
        { error: { message: "provider temporarily offline" } },
        { status: 503 },
      );
    const degraded = await answerAssetLens("critical assets", assets, "en");
    assert.equal(degraded.mode, "calculation");
    assert.match(degraded.limitation, /calculated directly from the asset register/);
  } finally {
    globalThis.fetch = originalFetch;
    if (previousKey === undefined) delete process.env.GEMINI_API_KEY;
    else process.env.GEMINI_API_KEY = previousKey;
...
test("OpenAI is an available server-side assistant provider without client-side secrets", async () => {
  ...
  globalThis.fetch = async (url, options) => {
    ...
    const output =
      calls === 1
        ...
        : {
            answer: "One critical asset is in poor condition in B1.",
            citations: [assets[0].id],
            caveat: "",
          };
    return Response.json({
      output: [
  ...
  try {
    const result = await answerAssetLens(
      "How many critical assets are poor in B1?",
      assets,
      "en",
    );
    assert.equal(calls, 2);
...
test("total asset counts bypass lexical search and do not require an AI provider", async () => {
  ...
  globalThis.fetch = async () => {
    calls += 1;
    throw new Error("AI must not be called for a deterministic total");
  };
  try {
    const english = await answerAssetLens(
      "how many total asset in my project?",
      assets,
      "en",
    );
    const arabic = await answerAssetLens(
      "كم عدد الأصول في مشروعي؟",
      assets,
      "ar",
    );
    ...
    assert.equal(
      (await answerAssetLens("how many critical assets?", assets, "en")).mode,
      "calculation",
    );
  } finally {
    globalThis.fetch = originalFetch;
