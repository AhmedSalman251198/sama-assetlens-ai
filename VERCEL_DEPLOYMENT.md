...
نفّذ `supabase/setup.sql` ثم migrations من `002` إلى `016` بالترتيب. `015` يضيف مساحة Asset Intelligence ورابط QR العام، و`016` يحصر البيانات المعروضة في QR ويضبط مستويات الموقع والصلاحيات داخل RLS. إذا شغّلت نسخة مبكرة من `015` فلا تتجاوز `016`. نفّذها أولًا في بيئة اختبار مع نسخة احتياطية، ثم اعتمدها في الإنتاج.
